...
func (s *SuperAgent) SetWorkerId(workerId string) *SuperAgent {
	s.WorkerId = workerId
	return s
}
...
func (s *SuperAgent) getResponseBytes() (Response, []byte, []error) {
	...
	if s.Debug {
		...
		if err != nil {
			s.logger.Error("message", "httputil.DumpRequest Error", "err", err.Error(), "worker-id", s.WorkerId)
		} else {
			s.logger.Info("message", "log detail of http request", "request", string(dump))
		}
	}
	...
	if s.Debug {
		...
		if nil != err {
			s.logger.Error("message", "HTTP Response Error", "err", err.Error(), "worker-id", s.WorkerId, "took", time.Since(start))
		} else {
			s.logger.Info("message", "HTTP Response", "response", string(dump), "worker-id", s.WorkerId, "took", time.Since(start))
		}
	}
